...
def filter_out!(arr, &prc)
  # if using each_with_index => delete_at
  # if using each_with_index => [0..i-1] + [i+1..-1] NOT MUTATION!
...
def proctition(arr, &prc)
  new_arr = []
  #trues are early in new array, falses are later
  # plan 1 : loop trues first then loop falses
  # plan 2 : falses use push, record a last_true_index and use insert!
  last_true_i = 0
  arr.each do |ele|
    if !prc.call(ele)
      new_arr << ele
    else #this is if prc is true
      new_arr.insert(last_true_i, ele)
      last_true_i += 1
    end
  end
  new_arr
